...
class Booking {
    ...
    public function updateBookingInfo($id,$movie,$date,$show_time,$theater,$seat_no,$no_of_tickets,$total_price,$customer_name,$customer_phone){
        $conn = Database::connect();
        $conn->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
        $sql = "UPDATE booking 
                SET movie_id=:movie,date=:date,show_time_id=:show_time,theater_id=:theater,seat_no=:seat_no,
                no_of_tickets=:no_of_tickets,total_price=:total_price,customer_name=:customer_name,customer_phone=:customer_phone
                WHERE id=:id";
        $statement = $conn->prepare($sql);
        $statement->bindParam(':id',$id);
        $statement->bindParam(':movie',$movie);
